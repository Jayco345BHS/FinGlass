...
from django_apps.core.services.tfsa_import_service import import_tfsa_transactions_rows, parse_tfsa_import_csv_text
from django_apps.core.services.tfsa_service import (
...
def tfsa_accounts(request):
    ...
    if request.method == "POST":
        ...
        try:
            account = TfsaAccount.objects.create(user_id=user_id, account_name=account_name, opening_balance=0)
...
def tfsa_summary(request):
    user_id = request.user.id
    summary = get_tfsa_summary(user_id)
    return JsonResponse(summary)


@require_http_methods(["GET", "PUT"])
def tfsa_opening_balance(request):
    user_id = request.user.id
    if request.method == "GET":
        balance = get_user_tfsa_opening_balance(user_id)
        base_year = get_user_tfsa_opening_balance_base_year(user_id)
        configured = is_user_tfsa_opening_balance_configured(user_id)
    ...
    set_user_tfsa_opening_balance(user_id, opening_balance)
...
def tfsa_annual_limits(request):
    ...
    if year < 2009 or year > 2100:
        return JsonResponse({"error": "year must be between 2009 and 2100"}, status=400)
    ...
    upsert_user_tfsa_annual_limit(user_id, year, annual_limit)
    return JsonResponse({"success": True}, status=201)


@require_http_methods(["DELETE"])
def tfsa_annual_limit_item(request, year):
    user_id = request.user.id
    deleted = delete_user_tfsa_annual_limit(user_id, year)
    if not deleted:
...
@require_http_methods(["GET"])
def tfsa_transactions(request):
    user_id = request.user.id
    rows = list(
        TfsaContribution.objects.filter(user_id=user_id)
        .select_related("tfsa_account")
        ...
        .values(
            ...
            "created_at",
            "tfsa_account_id",
            account_name=F("tfsa_account__account_name"),
        )
    )
...
def tfsa_transaction_item(request, transaction_id):
    user_id = request.user.id
    if request.method == "DELETE":
        deleted, _ = TfsaContribution.objects.filter(id=transaction_id, user_id=user_id).delete()
        if deleted == 0:
    ...
    try:
        ...
        return JsonResponse({"error": "tfsa_account_id must be an integer"}, status=400)
    ...
    transaction_row = TfsaContribution.objects.filter(id=transaction_id, user_id=user_id).first()
    if not transaction_row:
        return JsonResponse({"error": "Transaction not found"}, status=404)

    account = TfsaAccount.objects.filter(id=normalized_account_id, user_id=user_id).first()
    if not account:
    ...
    transaction_row.contribution_type = contribution_type
...
def tfsa_contributions(request):
    user_id = request.user.id
    data = _read_json(request)
    ...
    acc = TfsaAccount.objects.filter(id=tfsa_account_id, user_id=user_id).first()
    if not acc:
        return JsonResponse({"error": "Account not found"}, status=404)
    ...
    try:
        TfsaContribution.objects.create(
            user_id=user_id,
            tfsa_account_id=tfsa_account_id,
...
def tfsa_transfers(request):
    ...
    try:
        ...
        return JsonResponse({"error": "from_tfsa_account_id and to_tfsa_account_id must be integers"}, status=400)
...
@require_http_methods(["DELETE"])
def tfsa_account_item(request, account_id):
    user_id = request.user.id
    acc = TfsaAccount.objects.filter(id=account_id, user_id=user_id).first()
    if not acc:
        return JsonResponse({"error": "Account not found"}, status=404)
...
@require_http_methods(["POST"])
def tfsa_reset(request):
    user_id = request.user.id
    reset_user_tfsa_data(user_id)
    return JsonResponse({"success": True})


@require_http_methods(["POST"])
def tfsa_import_csv(request):
    user_id = request.user.id
    ...
    if not parsed_rows and setup_opening_balance is None and not setup_annual_limits:
        return JsonResponse({"error": "No TFSA rows found in uploaded CSV"}, status=400)
    ...
    with transaction.atomic():
        ...
        if setup_opening_balance is not None:
            set_user_tfsa_opening_balance(user_id, float(setup_opening_balance))
            setup_opening_balance_applied = True

        if setup_base_year is not None:
            set_user_tfsa_opening_balance_base_year(user_id, int(setup_base_year))
            setup_base_year_applied = True

        for annual_limit in setup_annual_limits:
            upsert_user_tfsa_annual_limit(user_id, int(annual_limit["year"]), float(annual_limit["annual_limit"]))
